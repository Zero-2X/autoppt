...
try:
    ...
    from .slide_auditor import audit_stage9, write_json as write_audit_json
...
try:
    from autosearch.presentation.orchestrator import (
        build_defense_materials,
        build_imagegen_manifest,
        prepare_presentation_package,
        run_presentation_gates,
    )
except ImportError:  # pragma: no cover - direct Stage9 execution from its folder.
...
from autosearch.presentation.profiles import get_presentation_profile
...
def update_workspace_state(
    ...
    *,
    ...
    state["current_stage"] = "stage9_ppt"
    state["current_stage_status"] = status
    state.setdefault("stage_status", {})["stage9_ppt"] = status
    state["active_focus"] = "Stage9 PPT/imagegen handoff complete" if status == "completed" else "Stage9 PPT/imagegen blocked"
    ...
    state["next_action"] = "pipeline_complete" if status == "completed" else "fix_stage9_ppt"
    ...
    state["blocking_reason"] = "" if status == "completed" else "Stage9 PPT audit failed."
    state["stage9_outputs"] = {
...
def render_mock_slide(path: Path, *, project_name: str, slide: dict[str, Any]) -> None:
    ...
    draw.text((1014, 642), "Stage9 mock", fill="#94A3B8", font=tag_font)
    img.save(path, quality=95)


def copy_stage9_outputs(stage45_dir: Path, final_ppt_dir: Path, image_prompts: dict[str, Any]) -> None:
...
def run_real_stage45_imagegen(stage45_dir: Path) -> None:
    ...
    output_path = stage45_dir / "pptx" / "output" / "stage9_competition_deck.pptx"
...
def run_stage9(
    ...
    *,
    ...
    if mock:
        ...
        stage45_pptx = output_dir / "stage9_competition_deck.pptx"
        ...
        if preflight_issues:
            ...
            return write_stage9_failure(
                topic_dir=topic_dir,
                final_ppt_dir=final_ppt_dir,
                image_prompts=image_prompts,
                project_name=project_name,
                reason="stage9_real_imagegen_preflight_failed",
        ...
        try:
            ...
            run_presentation_gates(topic_dir, package=package, prompts=image_prompts, stage45_dir=stage45_dir, final_deck=final_ppt_dir / "final_deck.pptx", mock=False)
            build_defense_materials(topic_dir, package=package, reports={})
            return write_stage9_failure(
                topic_dir=topic_dir,
                final_ppt_dir=final_ppt_dir,
    ...
    copy_stage9_outputs(stage45_dir, final_ppt_dir, image_prompts)
    ...
    sidecar_dir = stage45_dir / "assets" / "slides"
    ...
    write_json(stage45_dir / "image_generation_metadata.json", metadata_payload)
    write_json(final_ppt_dir / "image_generation_metadata.json", metadata_payload)
    write_json(stage45_dir / "imagegen_manifest.json", imagegen_manifest)
    ...
    write_json(final_ppt_dir / "presentation_run_manifest.json", {
        ...
        "stage9_audit": str(final_ppt_dir / "ppt_audit.json"),
        "validation_dir": str(final_ppt_dir / "validation"),
    ...
    workflow_log.write_text(
        "\n".join(
            [
                f"{datetime.now(timezone.utc).isoformat()} route={profile.get('imagegen_route')} profile={presentation_profile} stage9_audit={audit.get('verdict')}",
...
def regenerate_stage9_slide(
    ...
    *,
    ...
    Real mode temporarily narrows the Stage4.5 prompt pack to the requested
    ...
    stage45_dir = final_ppt_dir / "stage45_workspace"
    prompts_path = stage45_dir / "image-prompts.json"
    if not prompts_path.exists():
        raise FileNotFoundError(f"missing Stage4.5 prompt pack: {prompts_path}")
    ...
    project_name = load_project_name(topic_dir)
    target = stage45_dir / str(slide.get("final_path") or f"assets/slides/{slide_id}.png")
    ...
    if mock:
        ...
        try:
            ...
            try:
                prepare_handoff(
                    prompts_path,
                    output=stage45_dir / "single-slide-builtin-imagegen-handoff.json",
                )
            except Exception as exc:  # noqa: BLE001 - preserve exact handoff blocker.
                write_text(
                    stage45_dir / "references" / "single-slide-builtin-imagegen-handoff-prepare-error.txt",
                    f"{exc.__class__.__name__}: {exc}\n",
                )
            issues = preflight_real_imagegen(stage45_dir)
            if issues:
                report = write_builtin_blocker_report(stage45_dir, issues, image_prompts=narrowed)
                raise BuiltinImageGenBlocked(issues, report_path=report)
            run_real_stage45_imagegen(stage45_dir)
    ...
    write_json(stage45_dir / "imagegen_manifest.json", manifest)
