...
The historical wrapper name is retained for compatibility. It never invokes
an ImageGen API or CLI. Generation happens only through the Codex host's
...
def main() -> int:
    ...
    forbidden = {key: value for key, value in vars(args).items() if key not in {"stage", "output", "report", "gate_report"} and value not in (None, False, "")}
    if forbidden:
    ...
    stage, prompt_path = resolve_stage(args.stage)
    if not prompt_path.exists():
        raise SystemExit(f"Missing image prompt pack: {prompt_path}")
    output = Path(args.output).expanduser().resolve() if args.output else stage / "pptx" / "output" / f"{stage.name}.pptx"
    ...
    run([sys.executable, str(SCRIPT_DIR / "run_competition_imagegen.py"), str(prompt_path)])
    run([sys.executable, str(SCRIPT_DIR / "build_competition_ppt.py"), str(stage / "deck-spec.json"), "--output", str(output), "--report", str(report)])
    run([sys.executable, str(SCRIPT_DIR / "verify_imagegen_first_gate.py"), str(stage), "--pptx", str(output), "--require-strong-ig-id", "--report", str(gate_report)])
    print(f"\nDone: {output}")
    return 0
