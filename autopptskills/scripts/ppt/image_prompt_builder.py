...
PROMPT_POLICY: dict[str, Any] = {
    # This manifest is consumed by the built-in ImageGen handoff.  Keep the
    # backend and blocker behavior machine-checkable; no alternate provider
...
def build_prompt_zh(project_name: str, slide: dict[str, Any], slide_id: str, evidence_refs: list[str], style_profile: dict[str, Any] | None = None) -> str:
    ...
    profile_id = style.get("style_profile") or style.get("profile_id")
    ...
    if style_reference["official_source_inspired"]:
        ...
        slide_identity_guard = identity_prompt_guard(
            style_reference,
            slide_text_items(slide),
        )
        reference_boundary = (
            "\nOfficial-Source Reference Boundary:\n"
            "This is an official-source-inspired generic archetype, not an official template, affiliation, or endorsement.\n"
            f"Allowed extraction: {allowed}.\n"
...
Design density: {style['design_density']}; intentional minimal style: {str(style['intentional_minimal']).lower()}
Design completion: {style['design_completion']}
Quality boundary: {style['quality_guardrails']}{reference_boundary}
...
def build_image_prompts(
    *,
    ...
    verified_identity_text: list[str] | str | None = None,
    allowed_identity_text: list[str] | str | None = None,
) -> dict[str, Any]:
    ...
    validate_identity_text_exceptions(
        style,
        [item for slide in slides for item in slide_text_items(slide)],
    )
    prompt_policy = dict(PROMPT_POLICY)
    if style["style_reference"]["official_source_inspired"]:
        ...
        prompt_slides.append(
            {
                ...
                "headline": slide.get("title", slide_id),
                ...
                "intentional_minimal": style["intentional_minimal"],
                "design_completion": style["design_completion"],
                "quality_guardrails": style["quality_guardrails"],
                "style_reference": style["style_reference"],
                ...
                "prompt_zh": build_prompt_zh(project_name, slide, slide_id, evidence_refs, style),
                "prompt_en": "",
