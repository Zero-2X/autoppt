...
-> build_competition_ppt.py places those images into PPTX
```
...
`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `STAGE45_IMAGEGEN_CLI`,
`LOCAL_IMAGEGEN_COMMAND`, a provider SDK, a proxy, or a second image backend.
The Stage4.5 scripts may validate and assemble images that the built-in tool
...
Do not use `build_competition_ppt.py`, `compose_*pptx.py`, PIL, SVG, HTML, Canvas, matplotlib, or PowerPoint shapes to create final slide content before imagegen. Those tools are post-imagegen assembly or editable reconstruction tools, not a replacement for imagegen.
...
```text
topic_.../
  final/final_workbook.md or workspace/stage8_handoff/*定稿.md
  workspace/stage8_handoff/slide_brief.json
  workspace/stage8_handoff/ppt_outline.md
  workspace/stage8_handoff/visual-manifest.json
  workspace/stage2_idea_generation/evidence-ledger.json
```
...
### Stage4.5 full-slide imagegen workspace
...
```text
stage4_competition_ppt_generation/
  source-materials/
...
```powershell
python autopptskills\scripts\check_imagegen_backend.py --report $env:TEMP\imagegen-backend-check.json --timeout 60
```

Generate images and assemble:

```powershell
python autopptskills\scripts\run_stage45_imagegen_ppt.py <stage4_or_stage45_dir> --output <out.pptx> --report <build-report.json>
```
...
- If only assembly is needed, run `build_competition_ppt.py` directly only after verifying all image paths are fresh imagegen outputs for the current run.
